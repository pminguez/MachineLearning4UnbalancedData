...
#tags <- c("all","0-16","17-30","31-50","51-70","71-100","31-100")
#tags <- c("all","31-50","51-70","71-100","31-100")
...
input.file <- paste(data.dir, "COVID_4H_", variable.type,".", extra.tag, ".", tag, ".txt", sep="")
...
results.frame <- data.frame(TYPE=character(0), AGE_RANGE=character(0), CONDITIONS=character(0),
                            AUC=numeric(0),ACCURACY=numeric(0), 
                            SENSITIVITY=numeric(0), 
                            SPECIFICITY=numeric(0))

results.frame <- rbind(results.frame, data.frame(TYPE=variable.type, AGE_RANGE=tag, CONDITIONS=extra.tag,
                                                 AUC=round(mean(auc.vect), digits=2), 
...
results.file <- paste(results.dir, variable.type, ".", extra.tag, ".", tag, ".results.txt", sep="")
...
matrix.good <- matrix.frame[matrix.frame$OUTPUT == 0,]
matrix.bad <- matrix.frame[matrix.frame$OUTPUT == 1,]
p.vector <- c()
direction.vector <- c()
...
for(i in 1:length(levels.reordered)){
  #colors <- c()
  levels.reordered[i]
  my.index <- match(levels.reordered[i], colnames(matrix.frame))
  
 if(length(unique(matrix.frame[,my.index]))>2){
    ## Non-binary variable
    ...
    ## Binary variable
    good.drug <- sum(matrix.good[,my.index])
    good.nodrug <- length(matrix.good[,my.index]) - good.drug
    
    bad.drug <- sum(matrix.bad[,my.index])
    bad.nodrug <- length(matrix.bad[,my.index]) - bad.drug
    
    contingency.matrix <- matrix(c(good.drug, bad.drug, good.nodrug, bad.nodrug), nrow = 2,
                                 dimnames = list(c("GOOD", "BAD"), c("DRUG","NO_DRUG")))
...
results.file <- paste(results.dir, variable.type, ".", extra.tag, ".", tag, ".variables.significance.txt", sep="")
...
p1.file <- paste(results.dir, "importance.", variable.type, ".", extra.tag, ".", tag, ".results.png", sep="")
ggsave(p1.file, width=5, height = 4)
dev.off()
...
for(i in 1:length(levels.reordered)){
  ...
  if(significance.frame$median.direction.vector[i]>1){
    new.color <- "#9999FF"
    if(significance.frame$adjusted[i]<0.05){
      new.color <- "#0000FF"
    }
...
p2.file <- paste(results.dir, "importance.", variable.type, ".", extra.tag, ".", tag, ".median.results.png", sep="")
ggsave(p2.file, width=5, height = 4)
dev.off()
